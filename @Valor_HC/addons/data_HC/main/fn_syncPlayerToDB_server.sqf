...
switch (_side) do
{
	case opfor:
	{
		_query = format["Update players set gear_cop = '%1',cash = '%2', bankacc = '%3', stats_cop = '%4', alive_cop = '%5', position_cop = '%6', playtime = '%7', map = '%8' where pid = '%9'",_values select 0,_values select 1,_values select 2,_values select 3,_values select 4,_values select 5,_values select 6,_values select 7,_uid];
	};
