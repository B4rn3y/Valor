...
if((_base_id isEqualTo -1 || _config_id isEqualTo -1 || _layout_id isEqualTo -1 || _classname isEqualTo "") && (_cop isEqualTo 0)) exitWith {};
...

{
	...
	if!(_cop isEqualTo 1) then {
		...
		_object setVectorUp _vector;
		if(_classname in _base_box_classname) then {
			_object setvariable["cop",_base_id,true];
			_ret = [_object,_inv] call valor_fnc_loadVehicleCargo;
		};
	};
} foreach _res;
