...
_conf = [_config_id] call valor_fnc_outpost_config;
...
while {dialog} do
{
	...
	if(!_send && _all) then {
		_send = true;
		[[_config_id,_outpost_id,_container],"valor_fnc_changeOutpostStatus",2] call valor_fnc_remoteexec;
	};
