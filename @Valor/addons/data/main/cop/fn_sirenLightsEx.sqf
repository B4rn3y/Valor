...
_lightpos = param [1,[],[[]]];
_lightcolor = param [2,[],[[]]];
_siren = param[3,objNull,[objNull]];

if (count _lightpos isEqualTo 0 || count _lightcolor isEqualTo 0) exitWith {};
if (isNull _vehicle || !(_vehicle getVariable "lights")) exitWith {};
if(isnull _siren) exitWith {};
...
_lightLeft = "#lightpoint" createVehicleLocal getPos _siren;
waitUntil {!isNil "_lightLeft"};
_lightLeft setLightColor _lightColorLeft;
_lightLeft setLightBrightness 0;
_lightLeft lightAttachObject [_siren,_lightLeftPos];
...
_lightRight = "#lightpoint" createVehicleLocal getPos _siren;
waitUntil {!isNil "_lightRight"};
_lightRight setLightColor _lightColorRight;
_lightRight setLightBrightness 0;
_lightRight lightAttachObject [_siren, _lightRightPos];
...
// looping lights
[_lightLeft,_lightRight,_vehicle,_brightness,0.32,_siren] call valor_fnc_sirenLightsLoop;
