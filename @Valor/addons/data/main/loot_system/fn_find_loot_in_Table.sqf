...
private ["_chance","_loottable","_my_loot","_table"];
_chance = param[0,-1,[1]];
_loottable = param[1,"",[""]];
...
_table = switch (_loottable) do
{
	case "h_military":
	{
		selectRandom ["h_military","m_military","l_military"]
	};

	case "m_military":
	{
		selectRandom ["m_military","l_military"]
	};

	case "firestation":
	{
		selectRandom ["firestation","general"]
	};
...
{
	if((_x select 0) isequalto _table) then {
		{
